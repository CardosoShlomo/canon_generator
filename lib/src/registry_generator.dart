...
class RegistryGenerator extends GeneratorForAnnotation<Regents> {
  ...
  Future<String> generateForAnnotatedElement(
    ...
    for (final field in element.fields) {
      ...
      if (v != null) {
        ...
        facadeReads.add(
            '  UnitMemory<${vArgs.join(', ')}> get $name => ${name}Store;');
        continue;
      }
      ...
      if (g != null) {
        ...
        // The facade type is generated by THIS builder, so it resolves to
        // InvalidType in our own phase — recover the args as written in the
        // guard class's extends clause.
      ...
      facadeReads.add(
          '  StoreMemory<${args.join(', ')}> get $name => ${name}Store;');
    ...
    b.writeln('/// The read-only world a GUARD sees: one typed getter per store row.');
    b.writeln('/// Guards judge through this — never through the globals — so a judge');
    b.writeln('/// is replayable by construction.');
    b.writeln('class Stores {');
    b.writeln('  const Stores();');
    for (final r in facadeReads) {
      b.writeln(r);
    }
    b.writeln('}');
    b.writeln();
  ...
  /// The held object's `Guard<M,S>` (2 args) supertype, or null. A [Veto]
  /// matches through its Guard supertype.
  InterfaceType? _matchedGuard(DartType? t) {
    if (t is! InterfaceType) return null;
    for (final s in [t, ...t.allSupertypes]) {
      if (s.element.name == 'Guard' && s.typeArguments.length == 2) return s;
    }
    return null;
  }
  ...
  /// `extends` clause (`extends Veto<CachedChatsMsg, Stores>` counts — a
  /// Veto IS a Guard) — the fallback when the resolver reports InvalidType
  /// (the generated facade is hidden from this builder's own phase).

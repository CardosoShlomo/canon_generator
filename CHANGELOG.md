...
- Identity faces: a generated twin per id node (the capital-ID canon stamp) — `ProductID.of(context)` reads the ambient typed id, `ProductID.navOf(context)` mints the deictic handle, `screenOf`/`itemOf` read one source only; emitted only where the Flutter binding is imported, so pure-Dart consumers stay pure. Faces resolve BY NODE (verification by construction): `bind()` tags each store with its grammar node, every scope plants tagged, and a face read walks to the nearest matching plant — never another node's id.
- The gated tier: reverse composite verbs (`UserID.navOf(context).goAdChat()` — the other component read from the live chain) and the CLAIMED handle `UserID.on(context, chain)` — compile-gated by generated `<Node>On` markers on the chain classes (a chain that evidences nothing about the identity is a compile error, not a null).

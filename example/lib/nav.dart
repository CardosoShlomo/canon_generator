...
enum _Screens with ScreenNode<_Screens> {
  ...
  about(_Page('About')),
  account(_Page('Account'), Codec.string), //single-placement, id-bearing trunk
  ...
  static final graph = NavGraph(
    {
      ...
      // a historyless URL mirror. Set it via `Screen.on(.feed)!.query.category = ...`.
      feed.keep({
        item({editItem.inherit(item)}).query({_ItemKeys.sort(Codec.string)}),
      })
    ...
    // The boot UI — shown until the resolver commits the first screen, and the
    // face of the bare root `/`. While `Screen.root.kind` is set we're on a
    // floor: render its current front (or anything you like); else a spinner.
    root: const _Loading(),
...
// The boot / bare-root widget (rule 2): a plain widget, NOT a screen — it has
// no place in the nav tree. On a floor it shows the live front; else it boots.
class _Loading extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    if (Screen.root.kind != null) {
      return Screen.root.front ?? const Scaffold();
    }
      ...
      body: Center(
        child: Column(
          ...
          children: [
            ...
            // there's somewhere to pop (null at a trunk).
